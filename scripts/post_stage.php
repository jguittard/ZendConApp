 ...
 */
//replace doctrine place holders
require_once "deployment_params.inc";
$fileConfigStr = file_get_contents($appLocation . '/config/autoload/global.php');
foreach ($dbParams as $key => $value) {
    $fileConfigStr = str_replace($key, $value, $fileConfigStr);
}
file_put_contents($appLocation . '/config/autoload/global.php', $fileConfigStr);
